# 交接筆記（2026-09-04 更新:WiFi 可從手機遠端設定,B 已板上實測全程）
...
B 的四層 fallback(含手機熱點救援)已完整實測;**A 的熱點路徑還沒測**。
...
| 韌體版本 | **a11**(拉取式 OTA 裝入) | **v45**(拉取式 OTA 裝入) |
...
| 手機熱點救援 | ⚠️ **未實測**(程式碼與 B 相同) | ✅ 全程實測(09-03) |
...
- [ ] **A 的手機熱點路徑**。程式碼與 B 逐字相同,但 A 掉到熱點時**自己不會
      說話**,要靠 B 從 RTDB 讀到 `ssid` 變化再代發——而 `announceNodeMove()`
      **從來沒被觸發過**。要測的話跟 B 一樣:暫時把 A 的 `secrets.h` 換成假
      名字、發一版、`clear=1`
...
- `watchdog-v43` / `v44`:**測試版,secrets.h 被暫時換成連不上的假 SSID**
  以驗證救援熱點。secrets.h 是 gitignored,所以 repo 裡看不出差別——這兩個
  tag 的 .bin **不要拿來裝任何板子**,正式版從 v45 起
...
- [ ] **測 A 的手機熱點路徑**(見上方「還沒測的」)。B 測過了,但 A 靠 B 代發
      訊息這條路從沒觸發過
